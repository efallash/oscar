...
class OscarCommander(Node):
    def __init__(self):
        ...
        #Service Servers
        self.srv_left_arm=self.create_service(ArmControl, 'oscar/left_arm_command', self.left_arm_callback)
        self.srv_right_arm=self.create_service(ArmControl, 'oscar/right_arm_command', self.right_arm_callback)
        self.srv_left_gripper=self.create_service(GripperControl, 'oscar/left_gripper_command', self.left_gripper_callback)
        self.srv_right_gripper=self.create_service(GripperControl, 'oscar/right_gripper_command', self.right_gripper_callback)
        self.get_logger().info('Oscar Command Services are now available.')

        

    def left_arm_callback(self, request:ArmControl.Request, response:ArmControl.Response):
    ...
    def right_arm_callback(self, request:ArmControl.Request, response:ArmControl.Response):
    ...
    def left_gripper_callback(self, request:GripperControl.Request, response:GripperControl.Response):
        self.get_logger().info('Left gripper command received')
        success=self.gripper_control(request, 'left')
        return success

    def right_gripper_callback(self, request:GripperControl.Request, response:GripperControl.Response):
        self.get_logger().info('Right gripper command received')
        success=self.gripper_control(request, 'right')
        return success
    ...
    def arm_command(self, request:ArmControl.Request, arm):
        ...
        if len(request.named_pose)>0 :
            success, status = self.oscar.arm_go_to_named_pose(arm, request.named_pose, request.vel)
            
            if success:
                if execute:            
                    self.get_logger().info(f"Executed pose. Status code: {status}")
                else:
                    self.get_logger().info(f"Pose not executed. Planning code: {status}")
            else:
        ...
        for pitch in pitch_list:
            ...
            plan= self.oscar.plan(plan_comp, self.oscar.logger, single_plan_parameters=plan_params)
    ...
    def gripper_control(self, req, arm):
        pass
